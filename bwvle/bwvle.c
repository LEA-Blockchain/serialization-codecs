...
static int bwvle_write_bit(bwvle_encoder_t *enc, uint8_t bit) {
    if (enc->error_code != BWVLE_SUCCESS) return enc->error_code;
    if (enc->byte_pos >= enc->buffer_size) {
         return set_encoder_error(enc, BWVLE_ERROR_BUFFER_OVERFLOW);
    }
    if (enc->bit_pos == 7) { enc->buffer[enc->byte_pos] = 0; }
    if (bit) { enc->buffer[enc->byte_pos] |= (1 << enc->bit_pos); }
    if (enc->bit_pos == 0) { enc->bit_pos = 7; enc->byte_pos++; } else { enc->bit_pos--; }
...
static int bwvle_write_bits(bwvle_encoder_t *enc, uint64_t value, uint8_t num_bits) {
    if (enc->error_code != BWVLE_SUCCESS) return enc->error_code;
    if (num_bits > 64) return set_encoder_error(enc, BWVLE_ERROR_INVALID_DATA);
...
static int bwvle_read_bit(bwvle_decoder_t *dec) {
    if (dec->error_code != BWVLE_SUCCESS) return dec->error_code;
    ...
    if (dec->bit_pos == 0) { dec->bit_pos = 7; dec->byte_pos++; } else { dec->bit_pos--; }
...
static int bwvle_read_bits(bwvle_decoder_t *dec, uint8_t num_bits, uint64_t *value) {
    if (dec->error_code != BWVLE_SUCCESS) return dec->error_code;
    if (num_bits > 64) return set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA);
    *value = 0;
    for (uint8_t i = 0; i < num_bits; ++i) {
        int bit = bwvle_read_bit(dec);
        if (bit < 0) { return dec->error_code; }
        *value = (*value << 1) | bit;
    }
    return BWVLE_SUCCESS;
}
...
#else
    uint8_t bits = 0; uint64_t temp = v; while (temp > 0) { temp >>= 1; bits++; } return bits;
#endif
}
...
static int bwvle_encode_scalar_internal(bwvle_encoder_t *enc, uint64_t v) {
    ...
    for (uint8_t i = 0; i < n; ++i) { if (bwvle_write_bit(enc, 1) != BWVLE_SUCCESS) return enc->error_code; }
    if (bwvle_write_bit(enc, 0) != BWVLE_SUCCESS) return enc->error_code;
    if (bwvle_write_bits(enc, m, n) != BWVLE_SUCCESS) return enc->error_code;
    if (bwvle_write_bits(enc, v, m) != BWVLE_SUCCESS) return enc->error_code;
    return BWVLE_SUCCESS;
}
...
static int bwvle_decode_scalar_internal(bwvle_decoder_t *dec, uint64_t *value) {
    if (dec->error_code != BWVLE_SUCCESS) return dec->error_code;
    uint8_t n = 0; int bit;
    while ((bit = bwvle_read_bit(dec)) == 1) { n++; if (n > 64) return set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA); }
    if (bit < 0) return dec->error_code;
    if (n < 2) return set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA);
    uint64_t m_u64;
    if (bwvle_read_bits(dec, n, &m_u64) != BWVLE_SUCCESS) return dec->error_code;
    if (m_u64 > 64 || m_u64 == 0) return set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA);
    uint8_t m = (uint8_t)m_u64;
    uint64_t v;
    if (bwvle_read_bits(dec, m, &v) != BWVLE_SUCCESS) return dec->error_code;
    uint8_t m_check = bwvle_min_bits(v);
    if (m_check != m) return set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA);
    *value = v;
    return BWVLE_SUCCESS;
}
...
void bwvle_encoder_init(bwvle_encoder_t *enc, uint8_t *buffer, size_t buffer_size) {
    ...
    if (buffer_size > 0) memset(buffer, 0, buffer_size);
}

int bwvle_encode_scalar(bwvle_encoder_t *enc, uint64_t value) {
    if (enc->error_code != BWVLE_SUCCESS) return enc->error_code;
    if (bwvle_write_bit(enc, 1) != BWVLE_SUCCESS) return enc->error_code;
    if (bwvle_write_bit(enc, 1) != BWVLE_SUCCESS) return enc->error_code;
...
int bwvle_encode_bytes(bwvle_encoder_t *enc, const uint8_t *data, uint64_t length) {
     if (!enc) return BWVLE_ERROR_INVALID_DATA;
     if (enc->error_code != BWVLE_SUCCESS) return enc->error_code;
    ...
    if (bwvle_write_bit(enc, 1) != BWVLE_SUCCESS) return enc->error_code;
    if (bwvle_write_bit(enc, 1) != BWVLE_SUCCESS) return enc->error_code;
    if (bwvle_encode_scalar_internal(enc, length) != BWVLE_SUCCESS) return enc->error_code;


    for (uint64_t i = 0; i < length; ++i) {

        if (!data) return set_encoder_error(enc, BWVLE_ERROR_INVALID_DATA);
        if (bwvle_write_bits(enc, data[i], 8) != BWVLE_SUCCESS) return enc->error_code;
    }
    return BWVLE_SUCCESS;
}
...
size_t bwvle_encoder_finish(bwvle_encoder_t *enc) {
     if (!enc) return 0;
     if (enc->error_code != BWVLE_SUCCESS) return 0;
     if (enc->bit_pos != 7) {
         size_t bits_to_pad = enc->bit_pos + 1;
         for (size_t i = 0; i < bits_to_pad; ++i) {
             if (bwvle_write_bit(enc, 0) != BWVLE_SUCCESS) {
                  if (enc->error_code == BWVLE_ERROR_BUFFER_OVERFLOW &&
                      enc->byte_pos == enc->buffer_size && enc->bit_pos == 7) {
                      enc->error_code = BWVLE_SUCCESS;
                  }
                  if(enc->error_code != BWVLE_SUCCESS) return 0;
             }
...
void bwvle_decoder_init(bwvle_decoder_t *dec,
                        ...
                        void *user_data) {
    if (!dec || !buffer || !scalar_cb || !bytes_cb) return;
    dec->buffer = buffer;
...
int bwvle_decode(bwvle_decoder_t *dec) {
    ...
    while (dec->total_bits_consumed < buffer_total_bits) {
        ...
        if (bit1 == 0) {
             dec->total_bits_consumed--;
             if (dec->bit_pos == 7) { dec->byte_pos--; dec->bit_pos = 0; } else { dec->bit_pos++; }
            break;
        }
        if (dec->total_bits_consumed >= buffer_total_bits) { set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA); break; }
        int bit2 = bwvle_read_bit(dec);
        if (bit2 < 0) { if (dec->error_code == BWVLE_SUCCESS) set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA); break; }

        if (bit2 == 0) {
            uint64_t length;
            int len_bit1 = bwvle_read_bit(dec);
            int len_bit2 = bwvle_read_bit(dec);
            if (len_bit1 < 0 || len_bit2 < 0) { if (dec->error_code == BWVLE_SUCCESS) set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA); break; }
            if (len_bit1 != 1 || len_bit2 != 1) { set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA); break; }
            if (bwvle_decode_scalar_internal(dec, &length) != BWVLE_SUCCESS) break;
            ...
            if (length > 0 && (UINT64_MAX / 8) < length) { set_decoder_error(dec, BWVLE_ERROR_LENGTH_LIMIT_EXCEEDED); break; }
            uint64_t data_bits_needed = length * 8;


            if (data_bits_needed > buffer_total_bits - dec->total_bits_consumed) { set_decoder_error(dec, BWVLE_ERROR_BUFFER_OVERFLOW); break; }


            uint8_t* data_buf = NULL;
            if (length > 0) {
                 if (length > SIZE_MAX) { set_decoder_error(dec, BWVLE_ERROR_LENGTH_LIMIT_EXCEEDED); break; }
                 data_buf = (uint8_t*)malloc((size_t)length);
                 if (!data_buf) { set_decoder_error(dec, BWVLE_ERROR_ALLOCATION_FAILED); break; }
            }
            ...
            for (uint64_t i = 0; i < length; ++i) {
                uint64_t byte_val;
                if (bwvle_read_bits(dec, 8, &byte_val) != BWVLE_SUCCESS) { read_byte_error = 1; break; }
                if (data_buf) data_buf[i] = (uint8_t)byte_val;
            }
            if (read_byte_error) { free(data_buf); break; }
            ...
            uint64_t value;
            if (bwvle_decode_scalar_internal(dec, &value) != BWVLE_SUCCESS) break;
    ...
    if (dec->error_code == BWVLE_SUCCESS) {
        ...
        if (bits_remaining != expected_padding_bits) {
             ...
             for (size_t i = 0; i < expected_padding_bits; ++i) {
                 ...
                 if (pad_bit < 0) { if (dec->error_code == BWVLE_SUCCESS) set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA); break; }
                 if (pad_bit != 0) { set_decoder_error(dec, BWVLE_ERROR_INVALID_DATA); break; }
             }
        }
    }
